...
class BugBountyPlatform:
    ...
    def black_team_mode(self):
        ...
        platform.report_generator.print_summary(primary_target)
        ...
        platform.report_generator.generate_json_report(primary_target, json_file)
        platform.report_generator.generate_html_report(primary_target, html_file)
    ...
    def get_choice(self, prompt: str, max_choice: int, allow_back: bool = True) -> str:
        ...
        while True:
            try:
                ...
                if allow_back and choice.lower() in ['b', 'back', '0']:
                    return 'back'
...
║   {Colors.BRIGHT_GREEN}[4]{Colors.BRIGHT_WHITE}  🤖 AI Settings            - Ollama model configuration              ║
║   {Colors.BRIGHT_GREEN}[5]{Colors.BRIGHT_WHITE}  🔑 API Keys               - Bug bounty platform credentials         ║
║   {Colors.BRIGHT_GREEN}[6]{Colors.BRIGHT_WHITE}  📂 Output Directory       - Default report location                 ║
║   {Colors.BRIGHT_GREEN}[7]{Colors.BRIGHT_WHITE}  🔄 Reset to Defaults      - Restore default settings                ║
║   {Colors.BRIGHT_RED}[0]{Colors.BRIGHT_WHITE}  ⬅️  Back to Main Menu                                                ║
║                                                                               ║
╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}
"""
...
"""
    ...
    def view_findings(self):
        ...
        if findings:
            print(f"\n{Colors.BRIGHT_WHITE}   Recent Findings:{Colors.RESET}")
            print(f"   {'─' * 70}")
            ...
            for i, f in enumerate(findings[:10], 1):
                ...
                print(f"   {color}[{sev:>8}]{Colors.RESET} {f.get('vulnerability_type', 'Unknown')[:50]}")
                print(f"            {Colors.DIM}{f.get('target', 'N/A')[:60]}{Colors.RESET}")
                
            if len(findings) > 10:
                print(f"\n   {Colors.BRIGHT_YELLOW}... and {len(findings) - 10} more findings{Colors.RESET}")
    ...
    def manage_tools(self):
        ...
        while True:
            ...
            choice = self.get_choice("Select option (1-5):", 5)
    ...
    def manage_settings(self):
        ...
        while True:
            ...
            choice = self.get_choice("Select option (1-7):", 7)
            ...
            if choice == 'back':
                ...
                self.clear_screen()
                self.show_logo()
                print(f"\n{Colors.BRIGHT_CYAN}🔑 API KEY CONFIGURATION{Colors.RESET}")
                print(f"{'─' * 70}")
                print(f"\n{Colors.BRIGHT_WHITE}   Configure API keys for bug bounty platforms:{Colors.RESET}")
                print(f"\n   {Colors.BRIGHT_GREEN}[1]{Colors.RESET} HackerOne API Token")
                print(f"   {Colors.BRIGHT_GREEN}[2]{Colors.RESET} Bugcrowd API Token")
                print(f"   {Colors.BRIGHT_GREEN}[3]{Colors.RESET} Intigriti API Token")
                print(f"   {Colors.BRIGHT_RED}[0]{Colors.RESET} Back")
                
                key_choice = self.get_choice("Select platform:", 3)
                if key_choice != 'back':
                    token = input(f"\n{Colors.BRIGHT_YELLOW}➤ Enter API Token: {Colors.RESET}").strip()
                    if token:
                        print(f"\n   {Colors.BRIGHT_GREEN}✅ API token saved securely{Colors.RESET}")
    ...
    def run(self):
        ...
        while self.running:
            try:
                self.show_main_menu()
                choice = self.get_choice("Select option (1-10, 0 to exit):", 10, allow_back=False)
